...
class RangedEnemy(Enemy):
    ...
    def draw(self, screen):
        if self.health > 0:
            pygame.draw.circle(screen, BLUE, self.rect.center, RANGED_ENEMY_SIZE // 2)
        else:
            elapsed = pygame.time.get_ticks() - self.last_hit_time
            if elapsed < 1000:
                if (pygame.time.get_ticks() // 250) % 2 == 0:
                    pygame.draw.circle(screen, WHITE, self.rect.center, RANGED_ENEMY_SIZE // 2)
...
class Medkit:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, WHITE, (base_x, base_y, BOOSTER_ICON_SIZE, BOOSTER_ICON_SIZE))
        cross_w = 8
        cross_len = 30
        offset = (BOOSTER_ICON_SIZE - cross_len) // 2
        pygame.draw.rect(screen, RED, (base_x + offset, base_y + (BOOSTER_ICON_SIZE - cross_w)//2, cross_len, cross_w))
        pygame.draw.rect(screen, RED, (base_x + (BOOSTER_ICON_SIZE - cross_w)//2, base_y + offset, cross_w, cross_len))
...
class StaminaBooster:
    ...
    def draw(self, screen):
        ...
        base_x = self.rect.centerx - BOOSTER_ICON_SIZE // 2
        base_y = self.rect.centery - BOOSTER_ICON_SIZE // 2
        start = (base_x + 8, base_y + 8)
        end = (base_x + BOOSTER_ICON_SIZE - 8, base_y + BOOSTER_ICON_SIZE - 8)
        pygame.draw.line(screen, WHITE, start, end, 6)
        meat_center = (end[0] - 4, end[1] - 4)
        pygame.draw.circle(screen, BROWN, meat_center, 10)
        pygame.draw.circle(screen, RED, meat_center, 5)
...
class TransformBooster:
    ...
    def draw(self, screen):
        ...
        base_x = self.rect.centerx - BOOSTER_ICON_SIZE // 2
        base_y = self.rect.centery - BOOSTER_ICON_SIZE // 2
        mug_body = pygame.Rect(base_x + 5, base_y + 10, 28, 32)
        pygame.draw.rect(screen, BROWN, mug_body)
        pygame.draw.rect(screen, BLACK, mug_body, 2)
        pygame.draw.rect(screen, BLACK, (base_x + 34, base_y + 16, 8, 16))
        foam_rect = pygame.Rect(base_x + 4, base_y + 4, 30, 12)
        pygame.draw.ellipse(screen, YELLOW, foam_rect)
...
class Coin:
    ...
    def draw(self, screen):
        ...
        if now - self.spawn_time > COIN_LIFETIME - COIN_BLINK_START:
            if (now // 250) % 2 == 0:
                return
        pygame.draw.circle(screen, YELLOW, (int(self.x), int(self.y)), COIN_RADIUS)
...
class TavernNPC:
    ...
    def draw(self, screen):
        if self.type == 'trader':
            color = PINK if self.activated else PURPLE
        elif self.type == 'barman':
            color = WHITE if self.activated else BLACK
        pygame.draw.circle(screen, color, (self.x, self.y), self.radius)
        pygame.draw.circle(screen, WHITE if self.type != 'barman' else BLACK,
...
def init_level():
    ...
    obs = []
    obs.append(pygame.Rect(0, 0, WIDTH, WALL_THICKNESS))
    obs.append(pygame.Rect(0, HEIGHT - WALL_THICKNESS, WIDTH, WALL_THICKNESS))
    obs.append(pygame.Rect(0, 0, WALL_THICKNESS, door_y))
    obs.append(pygame.Rect(0, door_y + DOOR_HEIGHT, WALL_THICKNESS, HEIGHT - (door_y + DOOR_HEIGHT)))
    obs.append(pygame.Rect(WIDTH - WALL_THICKNESS, 0, WALL_THICKNESS, door_y))
    obs.append(pygame.Rect(WIDTH - WALL_THICKNESS, door_y + DOOR_HEIGHT, WALL_THICKNESS, HEIGHT - (door_y + DOOR_HEIGHT)))
    obs.append(right_door_rect)
    obstacles = obs + inner

    spawn_clear = pygame.Rect(WIDTH - WALL_THICKNESS - ENEMY_SIZE - 10,
                              door_y + DOOR_HEIGHT//2 - ENEMY_SIZE//2,
                              ENEMY_SIZE + 20, ENEMY_SIZE + 20)
    obstacles = [w for w in obstacles if not w.colliderect(spawn_clear)]

    player = pygame.Rect(70, door_y + DOOR_HEIGHT//2 - PLAYER_SIZE//2, PLAYER_SIZE, PLAYER_SIZE)
    # Стамина больше не сбрасывается при смене уровня (будет сохранена/восстановлена извне)
    # player_stamina = MAX_STAMINA  (убрано)
    ...
    # Трансформация сбрасывается только при начале новой игры; при переходе восстанавливается снаружи
    player_transformed = False
    transform_start_time = 0
    player_last_shot_time = 0
...
def init_tavern(final=False):
    ...
    obs.append(pygame.Rect(0, door_y + DOOR_HEIGHT, WALL_THICKNESS, HEIGHT - (door_y + DOOR_HEIGHT)))
    if final:
        obs.append(pygame.Rect(WIDTH - WALL_THICKNESS, 0, WALL_THICKNESS, HEIGHT))
    else:
        obs.append(pygame.Rect(WIDTH - WALL_THICKNESS, 0, WALL_THICKNESS, door_y))
        obs.append(pygame.Rect(WIDTH - WALL_THICKNESS, door_y + DOOR_HEIGHT, WALL_THICKNESS, HEIGHT - (door_y + DOOR_HEIGHT)))
    ...
    player = pygame.Rect(70, door_y + DOOR_HEIGHT//2 - PLAYER_SIZE//2, PLAYER_SIZE, PLAYER_SIZE)
    # Стамина сохраняется при переходе в таверну (не сбрасываем)
    # player_stamina = MAX_STAMINA  (убрано)
    ...
    # Сбрасываем все бустеры на игроке
    player_transformed = False
    transform_start_time = 0
    player_last_shot_time = 0
...
def start_new_game():
    ...
    player_lives = PLAYER_LIVES
    init_level()
    player_stamina = MAX_STAMINA   # полная стамина только в самом начале игры
...
while running:
    ...
    if game_state == MENU:
        ...
        if not player_moved and (dx != 0 or dy != 0 or space_pressed):
            player_moved = True
            if left_door_open:
                obstacles.append(left_door_rect)
        ...
        # Вражеская логика только во вражеской комнате
        if game_room_state == ROOM_ENEMY:
            # Спавн врагов волны
            if wave_active and not wave_all_dead and wave_spawned < WAVE_TOTAL and now >= wave_spawn_timer:
                if right_door_rect in obstacles:
                    obstacles.remove(right_door_rect)
                ...
                if not any(spawn_rect.colliderect(w) for w in obstacles):
                    ...
                    wave_spawned += 1
                obstacles.append(right_door_rect)
                right_door_open = False
                wave_spawn_timer = now + random.randint(WAVE_SPAWN_DELAY_MIN, WAVE_SPAWN_DELAY_MAX)
            ...
            # Переход при выходе через правую дверь
            if right_door_open and player.colliderect(right_door_rect):
                ...
                if enemy_rooms_completed == 10:
                    ...
                    if rooms_until_tavern <= 0:
                        ...
                        # Следующий вражеский уровень – сохраняем трансформацию
            ...
            # Выход из таверны (только если не финальная)
            ...
            if space_pressed:
                for npc in tavern_npcs:
                    ...
                    if attack_rect.colliderect(npc_rect):
                        npc.activated = True
                        if npc.type == 'trader':
                            # Пока только подсветка, в будущем торговля
                            pass
                        elif npc.type == 'barman':
                            if game_room_state == ROOM_FINAL_TAVERN:
                                # Победа при общении с барменом в финальной таверне
                                player_lives = PLAYER_LIVES
                                player_stamina = MAX_STAMINA
                                game_state = WIN
                            else:
        ...
        # Общие обновления (только для вражеской комнаты)
        if game_room_state == ROOM_ENEMY:
            ...
            if wave_active and not wave_all_dead:
                if now - last_medkit_spawn > random.randint(MEDKIT_SPAWN_INTERVAL_MIN, MEDKIT_SPAWN_INTERVAL_MAX) and not medkits:
                    while True:
                        ...
                        if not any(rect.colliderect(w) for w in obstacles):
        ...
        if left_door_open:
            pygame.draw.rect(screen, YELLOW, left_door_rect)
        if right_door_open:
            pygame.draw.rect(screen, YELLOW, right_door_rect)
        ...
        if game_room_state == ROOM_ENEMY:
            ...
            for tb in transform_boosters:
                tb.draw(screen)
            for b in bullets:
                pygame.draw.circle(screen, RED, b['rect'].center, BULLET_SIZE // 2)
        ...
        if game_room_state == ROOM_ENEMY and player_transformed:
            ...
            if elapsed < 30000 or (int((elapsed - 30000) / (1000/6)) % 2 == 0):
                color = WHITE if player_lives >= 3 else LIGHT_GRAY if player_lives == 2 else DARK_GRAY
                pygame.draw.circle(screen, color, player.center, PLAYER_RADIUS)
        else:
            color = WHITE if player_lives >= 3 else LIGHT_GRAY if player_lives == 2 else DARK_GRAY
